"""
GAM vs Transformer — 3-task benchmark
======================================
Tasks: 1-Gap Copy | 2-Running Sum | 3-Reverse
Single layer, ~54k params, causal LM.
"""
...
np.random.seed(SEED)
print(f"Device: {DEVICE}  d_model={D_MODEL}  epochs={EPOCHS}")
print("=" * 60)
...
# ═════════════════════════════════════════════════════════════════════
#  TASK 1 — GAP COPY  (delayed associative recall)
# ═════════════════════════════════════════════════════════════════════
print("\n[ TASK 1: GAP COPY ]")

V1 = 20          # 16 data tokens + BOS(16) + GAP(17) + PAD(18) + 1 extra
BOS1 = 16
GAP  = 17
PAD1 = 18
MIN_SEQ, MAX_SEQ = 4, 10
N_GAP = 8        # number of gap tokens before the sequence

def make_gapcopy(n):
    xs, ys = [], []
    for _ in range(n):
        L = np.random.randint(MIN_SEQ, MAX_SEQ + 1)
        seq = np.random.randint(0, 16, size=L).tolist()
        inp = [BOS1] + [GAP] * N_GAP + seq
        tgt = [GAP] * N_GAP + seq + [BOS1]
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 15 == 0 or ep == 1:
        print(f"  ep {ep:3d} | GAM train {l1:.4f} | Transformer train {l2:.4f}")

acc_gam_gc   = accuracy(gam_gc,   gc_test_x, gc_test_y, PAD1)
acc_trans_gc = accuracy(trans_gc, gc_test_x, gc_test_y, PAD1)
print(f"  >> Gap Copy Acc: GAM {acc_gam_gc:.4f}  |  Transformer {acc_trans_gc:.4f}")

# ═════════════════════════════════════════════════════════════════════
#  TASK 2 — RUNNING SUM  (working memory)
# ═════════════════════════════════════════════════════════════════════
print("\n[ TASK 2: RUNNING SUM ]")

V2 = 14          # digits 0-9(10) + BOS(10) + PLUS(11) + PAD(12) + 1 extra
BOS2 = 10
PLUS = 11
PAD2 = 12
DIGITS = 10
...
print(f"  >> Running Sum Acc: GAM {acc_gam_sum:.4f}  |  Transformer {acc_trans_sum:.4f}")

# ═════════════════════════════════════════════════════════════════════
#  TASK 3 — REVERSE  (long-range reordering)
# ═════════════════════════════════════════════════════════════════════
print("\n[ TASK 3: REVERSE ]")

V3 = 20          # 0-15 data + BOS(16) + REV(17) + PAD(18) + 1 extra
BOS3 = 16
REV  = 17
PAD3 = 18
MIN_REV, MAX_REV = 3, 8
...
def make_revdata(n):
    """In causal LM: model sees [BOS, t0..tn, REV, output-reversed...]"""
    xs, ys = [], []
    for _ in range(n):
        L = np.random.randint(MIN_REV, MAX_REV + 1)
        seq = np.random.randint(0, 16, size=L).tolist()
        rev = list(reversed(seq))
        inp = [BOS3] + seq + [REV] + rev
        tgt = seq + [REV] + rev + [BOS3]
        xs.append(inp); ys.append(tgt)
    return xs, ys
...
print(f"  >> Reverse Acc: GAM {acc_gam_rev:.4f}  |  Transformer {acc_trans_rev:.4f}")
