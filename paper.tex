...
The Transformer \citep{vaswani2017attention} has become the dominant
architecture for sequence modelling, powering large language models
(LLMs)~\citep{brown2020language}, vision transformers~\citep{dosovitskiy2020image},
...
\citep{katharopoulos2020transformers,katharopoulos2020linear},
state-space models~\citep{gu2023mamba},
recurrent neural networks~\citep{peng2023rwkv},
and fast-weight memories~\citep{schlag2021linear}.
...
\begin{itemize}[nosep]
  \item $\mathcal{O}(n)$ time and $\mathcal{O}(d^2)$ space per layer.
  \item ~4 projected matrices, one matrix recurrence, and a familiar
        residual-MLP block --- no softmax, no positional encoding.
...
\begin{itemize}[nosep]
  ...
  \item MLP weights: Xavier uniform~\citep{glorot2010understanding}.
...
We compare GAM against a single-head Transformer~\citep{vaswani2017attention}
with identical hyperparameters on three synthetic tasks.  Both models
use sinusoidal positional encodings and the same residual-MLP block
for fair comparison.
...
\textbf{Fast Weights.}
Schmidhuber~\citep{schmidhuber1992learning} proposed updating a
separate weight matrix at each step via an outer product.
Ba et al.~\citep{ba2016using} revived the idea for neural networks,
...
\textbf{DeltaNet.}
Schlag et al.~\citep{schlag2021linear} introduced a linear transformer
...
Katharopoulos et al.~\citep{katharopoulos2020transformers} replace
...
Mamba~\citep{gu2023mamba} and S4~\citep{gu2021efficiently} use
...
\textbf{RWKV.}
Peng et al.~\citep{peng2023rwkv} reformulate attention as a linear
